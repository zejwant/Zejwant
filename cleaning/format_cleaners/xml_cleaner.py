"""
cleaning/format_cleaners/xml_cleaner.py

Enterprise-level XML cleaning module.

Features:
- Parse XML tree and flatten
- Handle attributes and nested nodes
- Schema validation
- Missing value and type handling
- Deduplication and normalization
- Logging and error handling
- 20+ reusable cleaning methods
"""
...
logger = logging.getLogger("xml_cleaner")
...
def clean_xml(xml_file: str,
    ...
    """
    Enterprise-level XML cleaning.

    Args:
        xml_file (str): Path to XML file.
        root_path (str, optional): Root XPath to extract elements.
    ...
    try:
        ...
        root = tree.getroot() if not root_path else tree.find(root_path)

        # Flatten XML into DataFrame
        df = flatten_xml(root)
        ...
        logger.info(f"XML cleaned successfully, total rows: {len(df)}")
        return df
...
def flatten_xml(root: ET.Element) -> pd.DataFrame:
    """
    Flatten XML tree into a flat DataFrame.
    Includes attributes and nested elements with underscore naming.
    """
    rows = []

    def parse_element(elem: ET.Element, parent_path: str = '') -> Dict[str, Any]:
        data = {}
        path_prefix = f"{parent_path}_" if parent_path else ''
        # Attributes
        for k, v in elem.attrib.items():
            data[f"{path_prefix}{k}"] = v
        # Text
        if elem.text and elem.text.strip():
            data[f"{path_prefix}{elem.tag}"] = elem.text.strip()
        # Nested children
        for child in elem:
            data.update(parse_element(child, path_prefix + child.tag))
        return data

    for child in root:
        row = parse_element(child)
        rows.append(row)
